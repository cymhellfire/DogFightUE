...
void UCommonSessionSubsystem::SetHostSessionSetting(FName Key, FString& InValue)
{
	...
	HostSettings->Set(Key, InValue, EOnlineDataAdvertisementType::ViaOnlineService);
}

void UCommonSessionSubsystem::UpdateHostSessionSetting()
{
	if (!HostSettings.IsValid())
	{
		return;
	...
	Sessions->UpdateSession(FName(NAME_GameSession), *HostSettings);
}
